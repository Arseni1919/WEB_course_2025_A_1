...
def profiles_func(user_id):
    # check in DB if the user exists
    exist = True
    # exist = False
    if exist:
        # return render_template('profiles.html', user_id=user_id)
        return redirect(url_for('profiles_category_func', user_id=user_id, category='INFO-2'))
    return redirect('/')

@app.route('/profiles', defaults={'user_id': 100, 'category': 'INFO-1'})
@app.route('/profiles/<int:user_id>/<category>')
def profiles_category_func(user_id, category):
    # check in DB if the user exists
    exist = True
    # exist = False
    if exist:
        # return render_template('profiles.html', user_id=user_id, category=category)
        # return json.dumps({'user_id': user_id, 'category': category})
        return jsonify({'user_id': user_id, 'category': category})
    return redirect('/')
